  ...
  ScenarioOutline(
    ...
    ({ Given, When, Then, context, And }, variables) => {
      ...
      When(`initiate the token exchange`, async () => {
        const { code, code_verifier } = await authDriver.loginAndGetCode(
          context.user,
        );
        context.code = code;
      ...
      And(`the tokens should have correct validity period`, () => {
        ...
        const accessTokenResponse = jwtDecoderObj.decode(
          context.tokens.access_token,
        );
        const accessTokenValidityPeriod =
          jwtDecoderObj.calculateDuration(accessTokenResponse);
        expect(Math.abs(accessTokenValidityPeriod - 300)).toBeLessThanOrEqual(
          tolerance,
        ); //Allow test delay for 5 seconds

        const idTokenRes = jwtDecoderObj.decode(context.tokens.id_token);
        const idTokenValidityPeriod =
          jwtDecoderObj.calculateDuration(idTokenRes);
        expect(Math.abs(idTokenValidityPeriod - 3600)).toBeLessThanOrEqual(
          tolerance,
        ); //Allow test delay for 5 seconds
