...
const givenAnAppInitiatesLogin = (
...
const whenARequestIsMadeWithAttestationHeader = async ({
  ...
  if (variables['header'] === 'valid') {
    ...
    const { token } = await attestationDriver.getToken(
    ...
    context.user['attestationToken'] =
      mappedExamples[variables['header']]['attestationToken'];
  }
};
...
const thenRequestIsLogged = async ({
  ...
  const searchString =
    logMessageMap[variables['log'] as keyof typeof logMessageMap];
...
describeFeature(feature, ({ ScenarioOutline }) => {
  ...
  ScenarioOutline(
    ...
    ({ Given, When, Then, And, context }, variables) => {
      ...
      When(
        `the request is made to authenticate with <header> attestation header`,
        async () =>
          whenARequestIsMadeWithAttestationHeader({
            variables,
            context,
            attestationDriver,
          }),
      ...
      And(`the response message is <message>`, async () => {
        if (variables['message']) {
          expect(JSON.parse(context.response.statusText)?.message).toEqual(
            variables['message'],
          );
        }
      });
