...
describe('attestation lambda', () => {
  ...
  describe.skip('waf', () => {
    ...
    beforeAll(async () => {
      await repeatedlyRequestEndpoint(
        numRequests,
        requestFn,
        responseCodes,
        2000,
  ...
  describe.skipIf(!testConfig.isLocalEnvironment)('cloudwatch', () => {
    const alarmOKState = 'OK';
    const inAlarmState = 'ALARM';
    describe.each([
      [
        'given a high number of failed lambda invocations',
        testConfig.attestationLambdaErrorRateAlarmName,
      ],
      [
        'given a low proportion of 200 responses from attestation endpoint',
        testConfig.attestationLow200ResponseProportionAlarmName,
      ],
      [
        'given a low number of completed attestation requests',
        testConfig.attestationLowCompletionAlarmName,
      ],
    ])('%s', (_, alarmName) => {
      beforeAll(async () => {
      ...
      it('should trigger an alarm', async () => {
        const { MetricAlarms: updatedMetricAlarms } =
          await driver.performAction<DescribeAlarmsOutput>({
...
describe('api gateway', () => {
  it('should generate api gateway logs', async () => {
    ...
    expect(
      expectedKeys.every((key) => Object.keys(parsedLogs).includes(key)),
    ).toBe(true);
  });
});
