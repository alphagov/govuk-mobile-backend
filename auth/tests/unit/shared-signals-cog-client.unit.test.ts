...
describe('Test shared signal M2M cognito client', () => {
  ...
  it('should refer to cognito user pool', () => {
    expect(resourceUnderTest.Properties.UserPoolId.Ref).equal(
      'CognitoUserPool',
    );
  });

  it('has cognito as supported identity provider', () => {
    expect(resourceUnderTest.Properties.SupportedIdentityProviders).includes(
      'COGNITO',
    );
  });

  it('has correct Allowed OAuth Flow', () => {
    expect(resourceUnderTest.Properties.AllowedOAuthFlows).includes(
      'client_credentials',
    );
  });
