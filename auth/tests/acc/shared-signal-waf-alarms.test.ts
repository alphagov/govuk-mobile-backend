...
const testCases: AlarmTestCase[] = [
  {
    name: 'SharedSignalWafThrottlingAlarm',
    alarmName: `${testConfig.stackName}-shared-signal-waf-throttling-alarm`,
    actionsEnabled: true,
    metricName: 'BlockedRequests',
    alarmDescription: 'Alarm for throttling requests to the Shared Signal WAF',
...
describe.each(testCases)(
  `CloudWatch Alarm for Shared Signal WAF throttling $alarmName with supporting alarm resources`,
  async ({
    alarmName,
    actionsEnabled,
    metricName,
    ...
    it('should have Dimensions as an array', () => {
      ...
      assert.equal(rule?.Value, dimensions[1].Value); // WAF rule
    });
