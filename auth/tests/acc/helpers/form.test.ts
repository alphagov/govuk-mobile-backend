...
describe("Form parsing helper tests", () => {
    it("should parse a form tag from html", () => {
				...
				expect(formTag).toEqual(`<form action="/enter-email" method="post" novalidate>`);
    });
    it("should parse form attributes from a form tag", () => {
				const formData: FormData = parseFormAttributes(`<form action="/enter-email" method="post" novalidate>`);
