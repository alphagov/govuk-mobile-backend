...
import { SUPPORTED_AWS_SDK_CLIENTS, SUPPORTED_AWS_SDK_COMMANDS } from "./clients";

export const lambdaHandler = async (event: any): Promise<any> => {
    console.log(event)
    const { service, action, command } = event;

    // 1. Input Validation
    if (!service || !action || !command || typeof command.input === 'undefined') {
        console.error("Validation Error: Missing required fields in event payload.");
        return {
            error: 'InvalidInput',
            message: 'Event payload must contain "service", "action", and "command.input".',
            receivedEvent: event
        };
    }
    ...
    const client = new ClientConstructor({ region: process.env.AWS_REGION || 'us-east-1' });

    // 3. Validate and Instantiate AWS SDK Command
    const CommandConstructor = SUPPORTED_AWS_SDK_COMMANDS[service]?.[action];
    if (!CommandConstructor) {
        console.error(`Error: Unsupported action '${action}' for service '${service}'.`);
        return {
    ...
    try {
        ...
        console.log(`Executing AWS SDK command: ${service}.${action} with input:`, JSON.stringify(command.input, null, 2));
