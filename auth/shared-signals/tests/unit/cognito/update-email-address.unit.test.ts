...
describe("adminUpdateEmailAddress", () => {
  const userName = "test-user";
  const region = "eu-west-2";
  const newEmail = "tinkywinky@teletubbies.com";
  beforeAll(() => {
    process.env = {
      ...process.env,
      USER_POOL_ID: "123",
  ...
  it("returns true when email is successfully updated (HTTP 200)", async () => {
    sendMock.mockResolvedValue({ $metadata: { httpStatusCode: 200 } });
    const result = await adminUpdateEmailAddress(userName, newEmail);
  ...
  it("returns false when email update response is not 200", async () => {
