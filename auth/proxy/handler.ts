  ...
  try {
    ...
    switch (true) {
      case error instanceof ZodError:
        console.error(logMessages.ERROR_VALIDATION_ZOD, error.message, error.issues);
        return generateErrorResponse({
          statusCode: 400,
          message: 'Invalid Request'
        });
      case error instanceof TokenExpiredError:
        console.error(logMessages.ERROR_ATTESTATION_TOKEN_EXPIRED, error.message);
        return generateErrorResponse({
          statusCode: 401,
          message: 'Attestation token has expired'
        });
      case error instanceof JsonWebTokenError:
        console.error(logMessages.ERROR_ATTESTATION_JWT_INVALID, error.message);
        return generateErrorResponse({
          statusCode: 401,
          message: 'Attestation token is invalid'
        });
      case error instanceof UnknownAppError:
        console.error(logMessages.ERROR_ATTESTATION_APP_UNKNOWN, error.message);
        return generateErrorResponse({
          statusCode: 401,
          message: 'Unknown app associated with attestation token'
        });
      case error instanceof FailedToFetchSecretError:
        console.error(logMessages.ERROR_CONFIG_SECRET_FETCH_FAILED, error.message); 
        return generateErrorResponse({
          statusCode: 500,
