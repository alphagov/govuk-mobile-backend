...
describe('parseSecret', () => {
	...
	it.each([
		['{"client_secret": "user1", "password":}', `SyntaxError: Unexpected token '}', ..."password":}" is not valid JSON`],
		...
		[JSON.stringify({
			client_secret: ''
		}), `FailedToFetchSecretError: client_secret it empty`],
	])
	('should handle invalid inputs', (invalidSecret: any, message: string) => {
		expect(() => parseSecret(invalidSecret)).toThrow(new FailedToFetchSecretError(message));
	})
