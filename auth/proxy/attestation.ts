...
export interface AttestationUseCase {
  validateAttestationHeaderOrThrow: (
    headers: ReadonlyDeep<APIGatewayProxyEventHeaders>,
    path: string,
    config: Readonly<FirebaseConfig>) => Promise<void>
}
...
export const validateAttestationHeaderOrThrow = async (
  headers: Readonly<APIGatewayProxyEventHeaders>,
  path: string,
  config: Readonly<FirebaseConfig>
): Promise<void> => {
