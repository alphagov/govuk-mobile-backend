...
function cp_s3() {
  ...
  local TEMPLATE_MAJOR
  TEMPLATE_MAJOR=$(echo "${TEMPLATE_VERSION}" | cut -d "." -f 1)
  local TEMPLATE_MINOR
  TEMPLATE_MINOR=$(echo "${TEMPLATE_VERSION}" | cut -d "." -f 2)
  local TEMPLATE_PATCH
  TEMPLATE_PATCH=$(echo "${TEMPLATE_VERSION}" | cut -d "." -f 3)

  local metadata="repository=${GITHUB_REPOSITORY},commitsha=${GITHUB_SHA},version=${TEMPLATE_VERSION}"

  echo -e "\nPublishing stack '${stack_name}'..." 1>&2
  aws s3 cp "${stack_name}"/template.yaml "s3://${BUCKET_NAME}/${stack_name}/template.yaml" --metadata "${metadata}" --content-type "application/yaml" || status=$?
  aws s3 cp "${stack_name}"/template.yaml "s3://${BUCKET_NAME}/${stack_name}/template-${TEMPLATE_MAJOR}.yaml" --metadata "${metadata}" --content-type "application/yaml" || status=$?
  aws s3 cp "${stack_name}"/template.yaml "s3://${BUCKET_NAME}/${stack_name}/template-${TEMPLATE_MAJOR}.${TEMPLATE_MINOR}.yaml" --metadata "${metadata}" --content-type "application/yaml" || status=$?
  aws s3 cp "${stack_name}"/template.yaml "s3://${BUCKET_NAME}/${stack_name}/template-${TEMPLATE_MAJOR}.${TEMPLATE_MINOR}.${TEMPLATE_PATCH}.yaml" --metadata "${metadata}" --content-type "application/yaml" || status=$?
}
...
function sync_s3() {
  ...
  echo -e "\nPublishing stack '${stack_name}'..." 1>&2
  aws s3 sync --delete --quiet --exclude "*" --include "*template.yaml" "${stack_name}" "s3://${BUCKET_NAME}/${stack_name}" --metadata "${metadata}" --content-type "application/yaml" || status=$?
  aws s3 cp "${stack_name}"/template.yaml "s3://${BUCKET_NAME}/${stack_name}/template-${TEMPLATE_MAJOR}.yaml" --metadata "${metadata}" --content-type "application/yaml" || status=$?
  aws s3 cp "${stack_name}"/template.yaml "s3://${BUCKET_NAME}/${stack_name}/template-${TEMPLATE_MAJOR}.${TEMPLATE_MINOR}.yaml" --metadata "${metadata}" --content-type "application/yaml" || status=$?
  aws s3 cp "${stack_name}"/template.yaml "s3://${BUCKET_NAME}/${stack_name}/template-${TEMPLATE_MAJOR}.${TEMPLATE_MINOR}.${TEMPLATE_PATCH}.yaml" --metadata "${metadata}" --content-type "application/yaml" || status=$?
}
...
function _upload_to_s3() {
  ...
  local src_bucket_name="$2"
  local metadata="$3"
  local template_versions="$4"
  local TEMPLATE_MAJOR
  TEMPLATE_MAJOR=$(echo "${template_versions}" | cut -d "." -f 1)
  local TEMPLATE_MINOR
  TEMPLATE_MINOR=$(echo "${template_versions}" | cut -d "." -f 2)
  local TEMPLATE_PATCH
  TEMPLATE_PATCH=$(echo "${template_versions}" | cut -d "." -f 3)

  _go_into_app_dir "${app_to_deploy}"
  echo "Uploading to S3"
  aws s3 cp cf-template.yaml "s3://${src_bucket_name}/${app_to_deploy}/template.yaml" --metadata "${metadata}" --content-type "application/yaml" || status=$?
  aws s3 cp cf-template.yaml "s3://${src_bucket_name}/${app_to_deploy}/template-${TEMPLATE_MAJOR}.yaml" --metadata "${metadata}" --content-type "application/yaml" || status=$?
  aws s3 cp cf-template.yaml "s3://${src_bucket_name}/${app_to_deploy}/template-${TEMPLATE_MAJOR}.${TEMPLATE_MINOR}.yaml" --metadata "${metadata}" --content-type "application/yaml" || status=$?
  aws s3 cp cf-template.yaml "s3://${src_bucket_name}/${app_to_deploy}/template-${TEMPLATE_MAJOR}.${TEMPLATE_MINOR}.${TEMPLATE_PATCH}.yaml" --metadata "${metadata}" --content-type "application/yaml" || status=$?
}
...
function sync_sam_app() {
  ...
  _upload_to_s3 "${app_to_deploy}" "${BUCKET_NAME}" "${metadata}" "${TEMPLATE_VERSION}"
}
